...
public class DoLogin extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setCharacterEncoding("utf-8"); //post니까 한글처리 해야
		...
		try {
			...
			//데이터가 찾아지면, 즉 로그인이 되면.(while안쓰는 이유:어차피 데이터 하나만 찾아질 거니까.)
			if(rs.next()) { 
				name = rs.getString("name");
				
				//servlet에선 session이 내부객체가 아니라서 session.getAttribute만 써서 안됨. 선언 먼저 필요.
