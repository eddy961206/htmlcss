...
public class DoModify extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("잘되는지 테스트용 프린트1");
		...
		try {
			System.out.println("잘되는지 테스트용 프린트2");
			Class.forName("oracle.jdbc.driver.OracleDriver");
			conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","orauser","1111");
			stmt = conn.createStatement();
			String query = "update member set phone='"+phone+"',gender='"+gender+"',hobby='"+hobby+"' where id='"+id+"' ";  //원래는 패스워드거 맞아야 수정이 가능해야.
			...
			System.out.println("잘되는지 테스트용 프린트3");
			writer.close();
